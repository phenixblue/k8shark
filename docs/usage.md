...
| Component | Supported |
|---|---|
| **Kubernetes server** (for `capture`) | **Verified v1.30–v1.36** as of `v1.0.0`; **continuously tested** on `v1.36.1` only. Tracked via `k8s.io/client-go` in `go.mod` (currently `v0.36.x`), with the pinned minor validated on every change by the [conformance workflow](../.github/workflows/conformance.yml). k8shark captures through the generic REST/discovery surface rather than version-specific APIs, which is why the range is this wide — see the [version window](stability-policy.md#supported-kubernetes--kubectl-version-window) for what "verified" covers and how to reproduce it. |
