...
In addition, every capture records a **columns-only** Table schema for each list-capable native (built-in) kind that isn't otherwise targeted, stored at the kind's cluster-scoped list path with a `?as=TableSchema` suffix. These records contain the server's `columnDefinitions` with the `rows` array stripped — no object data — so the replay server can render kubectl-accurate columns (and `-o wide`) for objects it has no full Table for (e.g. writable-overlay creations or untargeted kinds) without persisting data for kinds the user didn't capture. Custom resources are excluded (their columns come from the CRD's `additionalPrinterColumns`). Both `?as=Table` and `?as=TableSchema` are k8shark-internal conventions and never appear in real API URLs.
